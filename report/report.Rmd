...
formula1 <- as.formula(paste("y.train ~", paste0("1+",paste(colnames(X1.train), collapse = "+")),"+(",paste(colnames(X1.train), collapse = "+"),"|title)"))

formula2 <- as.formula(paste("y.train ~", paste0("1+",paste(colnames(X2.train), collapse = "+")),"+(",paste(colnames(X2.train), collapse = "+"),"|pClass)"))

formula3 <- as.formula(paste("y.train ~", paste0("1+",paste(colnames(X3.train), collapse = "+")),"+(",paste(colnames(X3.train), collapse = "+"),"|ticket)"))

formula4 <- as.formula(paste("y.train ~", paste0("1+",paste(colnames(X4.train), collapse = "+")),"+(",paste(colnames(X4.train), collapse = "+"),"|embarked)"))

formula5 <- as.formula(paste("y.train ~", paste0("1+",paste(colnames(X5.train), collapse = "+")),"+(",paste(colnames(X5.train), collapse = "+"),"|fsize)"))

multi_hier1_title_prior.fit <- stan_glmer(formula = formula1,
                              data = df1.train,
                              cores=4,
                              adapt_delta = 0.99,
                              algorithm = "sampling",
...
prop_zero_test1 <- pp_check(multi_hier1_title_prior.fit, plotfun = "stat", stat = "prop_zero")
sd_zero_test1 <- pp_check(multi_hier1_title_prior.fit, plotfun = "stat", stat = "sd_zero")
grid.arrange(prop_zero_test1, sd_zero_test1, ncol=1)
```
...
# brms package trial
formula1_brms <- as.formula(paste("y.train ~", paste0("1+",paste(colnames(df1.train)[2:(length(colnames(df1.train))-1)], collapse = "+")),"+(0+",paste(colnames(df1.train)[2:(length(colnames(df1.train))-1)], collapse = "+"),"|title)"))

formula1_brms <- bf(formula = formula1_brms, center = F)

prior1 <- prior(student_t(3,0,2.5), class = b) +
    prior(lkj(0.5), class = cor)

multi_hier1_title_post.fit_brms <- brm(formula = formula1_brms,
                                       data = df1.train,
                                       prior = prior1,
...
stancode(multi_hier1_title_post.fit_brms)
prior_summary(multi_hier1_title_post.fit_brms)
summary(multi_hier1_title_post.fit_brms)
conditional_effects(multi_hier1_title_post.fit_brms)
