...
formula1 <- as.formula(paste("y.train ~", paste0("1+",paste(colnames(df1.train)[2:(length(colnames(df1.train))-1)], collapse = "+")),"+(0+",paste(colnames(df1.train)[2:(length(colnames(df1.train))-1)], collapse = "+"),"|title)"))

formula1 <- bf(formula = formula1, center = F)

prior1 <- prior(student_t(3,0,2.5), class = b) +
    prior(lkj(2), class = cor)

multi_hier1_title_prior.fit <- brm(formula = formula1,
                                       data = df1.train,
                                       prior = prior1,
                                       sample_prior = "only",
...
pp_check(multi_hier1_title_prior.fit_brms, type = "stat_2d")
pp_check(multi_hier1_title_prior.fit_brms, type = "violin_grouped", group = "title")
```
...
# model 1: hierarchical grouped by title with brms---------------------------------------------------------------
multi_hier1_title_post.fit <- brm(formula = formula1_brms,
                                       data = df1.train,
                                       prior = prior1,
                                       family = bernoulli(link="logit"),
                                       cores = parallel::detectCores(),
...
yrep.posterior1 <- posterior_predict(multi_hier1_title_post.fit)
```
...
pp_check(y.train, yrep1, ppc_dens_overlay)
pp_check(y.train, yrep1, fun = "stat_grouped", group = title, stat = "mean")
pp_check(y.train, yrep1, ppc_bars)
pp_check(y.train, yrep1, ppc_bars_grouped, group = title)
pp_check(y.train, yrep1, ppc_rootogram)
...
mcmc_intervals(multi_hier1_title_post.fit_brms, pars = c("b_Intercept",paste0("b_",colnames(df1.train)[2:(length(colnames(df1.train))-1)])))

mcmc_areas(multi_hier1_title_post.fit_brms, pars = c("b_Intercept",paste0("b_",colnames(df1.train)[2:(length(colnames(df1.train))-1)])))
