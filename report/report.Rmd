...
# Log transform continuous fare variabæe
...
summary(model2_fit_hier_title)
prior_summary(object = model2_fit_hier_title)

# assign to object and customize with functions from ggplot2
(gg1 <- posterior_vs_prior(model1_fit_hier_pClass, pars = c("beta", "varying"), prob = 0.95))
(gg2 <- posterior_vs_prior(model2_fit_hier_title, pars = c("beta", "varying"), prob = 0.95))
gg1 +
...
mcmc_areas(as.matrix(model1_fit_hier_pClass)[,1:length(colnames(df1.train))])
mcmc_areas(as.matrix(model2_fit_hier_title)[,1:length(colnames(df2.train))])
```
