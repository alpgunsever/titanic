...
Another step to take before getting into the imputation is to look at the available data visually. As a starting point to that, the first plot on below left shows the average age for each title. The one on the right shows the right shows the number of observations for each title:

```{r visual check with available data1, cache=TRUE, echo=FALSE}
...
The average age for the title make sense with Master having the lowest average age and Soldier, which probably represents the seniors that have been in military at some point. It can also be seen that most of the passengers are middle aged men and there are few of nobles and soldiers. The next two graphs below show the relationship between the title and the survival rate:

```{r visual check with available data2, cache=TRUE, echo=FALSE}
