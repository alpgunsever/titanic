...
```{r libraries, cache=TRUE, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
library(stringr)
library(corrplot)
library(projpred)
library(bayesplot)
...
We will start model fitting with 7 different models which are decided on a partially random logic. The descriptions of the models are below:
...
* **Model 5:** All predictors model with population and group level effects using a student-t(7,0,2.5) prior and a lkj(1) for the covariance matrix of the parameters variation. The grouping variable is title.
* **Model 6:** All predictors model with population and group level effects using a student-t(7,0,2.5) prior and a lkj(1) for the covariance matrix of the parameters variation. The grouping variable is pClass.
* **Model 7:** All predictors model with population and group level effects using a student-t(7,0,2.5) prior and a lkj(1) for the covariance matrix of the parameters variation. The grouping variable is Cabin.

```{r model fitting, cache=TRUE, eval=TRUE, echo=FALSE, message=FALSE, warning = FALSE,error=FALSE, results="hide"}
title <- titleTrain
...
formula_hier3_Cabin <- as.formula(paste("y.train ~", paste0("1+",paste(colnames(df3.train)[2:(length(colnames(df3.train))-1)], collapse = "+")),"+(1+",paste(colnames(df3.train)[2:(length(colnames(df3.train))-1)], collapse = "+"),"|Cabin)"))
formula_hier3_Cabin <- bf(formula = formula_hier3_Cabin, center = F)
...
multi_hier2_pClass_post.fit <- brm(formula = formula_hier2_pClass,
                                       ...
                                       save_pars = save_pars(all = TRUE),
                                       seed = 187)
# model 7: All predictors model with population and group effects by Cabin----------------------------
multi_hier3_Cabin_post.fit <- brm(formula = formula_hier3_Cabin,
                                       data = df3.train,
                                       prior = prior0,
                                       family = bernoulli(link="logit"),
                                       cores = parallel::detectCores(),
...
After modeling is completed, we can initially look at the bayesian generalization of the R2 coefficient to be able to investigate how much variation in the response variables can be explained by our models below. 

It seems like the Model 5 is able to explain most of the variation in the response variable which is around 53%. Model 1 doesn't seem to be able to explain any of the variation.

```{r diagnostics0, cache=TRUE, eval=TRUE, echo=FALSE}
bayes_R2_dataframe <- data.frame(Model = as.character(), R2.Estimate = as.numeric(), est.Error = as.numeric())

bayes_R2_dataframe[1,1] <- "Model1"
bayes_R2_dataframe[1,2] <- round(bayes_R2(only_population_intercept.fit)[1],2)
bayes_R2_dataframe[1,3] <- round(bayes_R2(only_population_intercept.fit)[2],2)    

bayes_R2_dataframe[2,1] <- "Model2"
bayes_R2_dataframe[2,2] <- round(bayes_R2(population_all.fit)[1],2)
bayes_R2_dataframe[2,3] <- round(bayes_R2(population_all.fit)[2],2) 

bayes_R2_dataframe[3,1] <- "Model3"
bayes_R2_dataframe[3,2] <- round(bayes_R2(population_all_hs.fit)[1],2)
bayes_R2_dataframe[3,3] <- round(bayes_R2(population_all_hs.fit)[2],2) 

bayes_R2_dataframe[4,1] <- "Model4"
bayes_R2_dataframe[4,2] <- round(bayes_R2(population_and_group_intercept.fit)[1],2)
bayes_R2_dataframe[4,3] <- round(bayes_R2(population_and_group_intercept.fit)[2],2) 

bayes_R2_dataframe[5,1] <- "Model5"
bayes_R2_dataframe[5,2] <- round(bayes_R2(multi_hier1_title_post.fit)[1],2)
bayes_R2_dataframe[5,3] <- round(bayes_R2(multi_hier1_title_post.fit)[2],2) 

bayes_R2_dataframe[6,1] <- "Model6"
bayes_R2_dataframe[6,2] <- round(bayes_R2(multi_hier2_pClass_post.fit)[1],2)
bayes_R2_dataframe[6,3] <- round(bayes_R2(multi_hier2_pClass_post.fit)[2],2)

bayes_R2_dataframe[7,1] <- "Model7"
bayes_R2_dataframe[7,2] <- round(bayes_R2(multi_hier3_Cabin_post.fit)[1],2)
bayes_R2_dataframe[7,3] <- round(bayes_R2(multi_hier3_Cabin_post.fit)[2],2)

bayes_R2_dataframe %>% kbl() %>% kable_minimal()
```

In the next section, we will perform posterior predictive checks and will try to validate our models in more depth.

## 5 - Posterior Predictive Checking

```{r simulating posterior predictions, cache=TRUE, eval=TRUE, echo=FALSE}
...
yrep7.posterior <- posterior_predict(multi_hier3_Cabin_post.fit)
```

Initially we will plot the first 50 simulations of the posterior predictive distribution below and compare it with the actual reponse variable distribution like we did in the prior predictive check section:

```{r posterior predictive checking 1, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 2.5, message=FALSE}
pp1 <- pp_check(y.train, yrep1.posterior[1:50,], ppc_dens_overlay) + labs(title = "Model1")
```

```{r posterior predictive checking 2, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 2.5, message=FALSE}
pp2 <- pp_check(y.train, yrep2.posterior[1:50,], ppc_dens_overlay) + labs(title = "Model2")
```

```{r posterior predictive checking 3, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 2.5, message=FALSE}
pp3 <- pp_check(y.train, yrep3.posterior[1:50,], ppc_dens_overlay) + labs(title = "Model3")
```

```{r posterior predictive checking 4, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 5, message=FALSE}
pp4 <- pp_check(y.train, yrep4.posterior[1:50,], ppc_dens_overlay) + labs(title = "Model4")
grid.arrange(pp1,pp2,pp3,pp4, ncol=2)
```

```{r posterior predictive checking 5, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 2.5, message=FALSE}
pp5 <- pp_check(y.train, yrep5.posterior[1:50,], ppc_dens_overlay) + labs(title = "Model5")
```

```{r posterior predictive checking 6, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 2.5, message=FALSE}
pp6 <- pp_check(y.train, yrep6.posterior[1:50,], ppc_dens_overlay) + labs(title = "Model6")
```

```{r posterior predictive checking 7, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 5, message=FALSE}
pp7 <- pp_check(y.train, yrep7.posterior[1:50,], ppc_dens_overlay) + labs(title = "Model7")

grid.arrange(pp5,pp6,pp7, ncol=2)
```

It's not possible to distinguish the 50 simulations of each model from each other but it can be seen that the model parameters have converged to reflect a similar shape to the actual response variable modeled with logistic regression. 

We can next look at the mean and standard deviation of the models in comparison to the actual below:

```{r posterior predictive checking 8, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 2.5, message=FALSE}
ppmsd11 <- ppc_stat(y.train, yrep1.posterior, stat = "mean")
ppmsd12 <-ppc_stat(y.train, yrep1.posterior, stat = "sd")
grid.arrange(ppmsd11,ppmsd12, ncol=2, top = "Model1")
```

```{r posterior predictive checking 9, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 2.5, message=FALSE}
ppmsd21 <- ppc_stat(y.train, yrep2.posterior, stat = "mean")
ppmsd22 <-ppc_stat(y.train, yrep2.posterior, stat = "sd")
grid.arrange(ppmsd21,ppmsd22, ncol=2, top = "Model2")
```

```{r posterior predictive checking 10, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 2.5, message=FALSE}
ppmsd31 <- ppc_stat(y.train, yrep3.posterior, stat = "mean")
ppmsd32 <-ppc_stat(y.train, yrep3.posterior, stat = "sd")
grid.arrange(ppmsd31,ppmsd32, ncol=2, top = "Model3")
```

```{r posterior predictive checking 11, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 2.5, message=FALSE}
ppmsd41 <- ppc_stat(y.train, yrep4.posterior, stat = "mean")
ppmsd42 <-ppc_stat(y.train, yrep4.posterior, stat = "sd")
grid.arrange(ppmsd41,ppmsd42, ncol=2, top = "Model4")
```

For hierarchical models (Model 5,6 & 7), we will plot mean and standard deviations for each group in the model:

```{r posterior predictive checking 12, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 5, message=FALSE}
ppmsd51 <- ppc_stat_grouped(y.train, yrep5.posterior, group = train_imp$title, stat = "mean")
ppmsd52 <- ppc_stat_grouped(y.train, yrep5.posterior, group = train_imp$title, stat = "sd")
grid.arrange(ppmsd51,ppmsd52, ncol=1, top = "Model5")
```

```{r posterior predictive checking 13, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 5, message=FALSE}
ppmsd61 <- ppc_stat_grouped(y.train, yrep6.posterior, group = train_imp$Pclass, stat = "mean")
ppmsd62 <- ppc_stat_grouped(y.train, yrep6.posterior, group = train_imp$Pclass, stat = "sd")
grid.arrange(ppmsd61,ppmsd62, ncol=1, top = "Model6")
```

```{r posterior predictive checking 14, cache=TRUE, eval=TRUE, echo=FALSE, fig.height = 5, message=FALSE}
ppmsd71 <- ppc_stat_grouped(y.train, yrep7.posterior, group = train_imp$Cabin, stat = "mean")
ppmsd72 <- ppc_stat_grouped(y.train, yrep7.posterior, group = train_imp$Cabin, stat = "sd")
grid.arrange(ppmsd71,ppmsd72, ncol=1, top = "Model7")
```

The mean and standard deviations of the models seem to have converged to mimic the response variable distribution. Next we can look into the classification accuracy and balanced classification accuracy of each model below: 

```{r posterior predictive checking 15, cache=TRUE, eval=TRUE, echo=FALSE}
# posterior classification accuracy for each model  
pred1 <- colMeans(yrep1.posterior)
pred2 <- colMeans(yrep2.posterior)
pred3 <- colMeans(yrep3.posterior)
pred4 <- colMeans(yrep4.posterior)
pred5 <- colMeans(yrep5.posterior)
pred6 <- colMeans(yrep6.posterior)
pred7 <- colMeans(yrep7.posterior)
...
pr7 <- as.integer(pred7 >= 0.5)

model1_pca <- round(mean(xor(pr1,as.integer(y.train==0))),2)
model2_pca <- round(mean(xor(pr2,as.integer(y.train==0))),2)
model3_pca <- round(mean(xor(pr3,as.integer(y.train==0))),2)
model4_pca <- round(mean(xor(pr4,as.integer(y.train==0))),2)
model5_pca <- round(mean(xor(pr5,as.integer(y.train==0))),2)
model6_pca <- round(mean(xor(pr6,as.integer(y.train==0))),2)
model7_pca <- round(mean(xor(pr7,as.integer(y.train==0))),2)

# posterior balanced classification accuracy for each model
model1_bpca <- round((mean(xor(pr1[y.train==0]>0.5,as.integer(y.train[y.train==0])))+mean(xor(pr1[y.train==1]<0.5,as.integer(y.train[y.train==1]))))/2,2)
model2_bpca <- round((mean(xor(pr2[y.train==0]>0.5,as.integer(y.train[y.train==0])))+mean(xor(pr2[y.train==1]<0.5,as.integer(y.train[y.train==1]))))/2,2)
model3_bpca <- round((mean(xor(pr3[y.train==0]>0.5,as.integer(y.train[y.train==0])))+mean(xor(pr3[y.train==1]<0.5,as.integer(y.train[y.train==1]))))/2,2)
model4_bpca <- round((mean(xor(pr4[y.train==0]>0.5,as.integer(y.train[y.train==0])))+mean(xor(pr4[y.train==1]<0.5,as.integer(y.train[y.train==1]))))/2,2)
...
model6_bpca <- round((mean(xor(pr6[y.train==0]>0.5,as.integer(y.train[y.train==0])))+mean(xor(pr6[y.train==1]<0.5,as.integer(y.train[y.train==1]))))/2,2)
model7_bpca <- round((mean(xor(pr7[y.train==0]>0.5,as.integer(y.train[y.train==0])))+mean(xor(pr7[y.train==1]<0.5,as.integer(y.train[y.train==1]))))/2,2)

model_classification <- data.frame(ModelName = as.character(), ClassAcc = as.numeric(), BalancedClassAcc = as.numeric())
model_classification[1,1] <- "Model1"
model_classification[1,2] <- model1_pca
model_classification[1,3] <- model1_bpca
model_classification[2,1] <- "Model2"
model_classification[2,2] <- model2_pca
model_classification[2,3] <- model2_bpca
model_classification[3,1] <- "Model3"
model_classification[3,2] <- model3_pca
model_classification[3,3] <- model3_bpca
model_classification[4,1] <- "Model4"
model_classification[4,2] <- model4_pca
model_classification[4,3] <- model4_bpca
model_classification[5,1] <- "Model5"
model_classification[5,2] <- model5_pca
model_classification[5,3] <- model5_bpca
model_classification[6,1] <- "Model6"
model_classification[6,2] <- model6_pca
model_classification[6,3] <- model6_bpca
model_classification[7,1] <- "Model7"
model_classification[7,2] <- model7_pca
model_classification[7,3] <- model7_bpca

model_classification %>% kbl() %>% kable_styling(latex_options = "hold_position")
```

It can be seen that the predictive performance of the models are way too over-optimistic. In the next section, we will try to improve the existing models as well as creating new ones to check if we can improve performance.

## 6 - Additional Models and Model Improvements

We will again start semi randomly and try to improve our models. The summary of the improvements are as below:

* **Model 8:** Interaction terms added to all predictors with population effects only model. This will be an improvement to Model 2.
* **Model 9:** Interaction terms added to all predictors with hierarchical model grouped by title. This will be an improvement to Model 5.
* **Model 10:** Interaction terms added to all predictors with hierarchical model grouped by pClass. This will be an improvement to Model 6.
* **Model 11:** Interaction terms added to all predictors with hierarchical model grouped by Cabin. This will be an improvement to Model 7.
* **Model 12:** Double grouping all the predictors for hierarchical model with title and embarked.
* **Model 13:** Double grouping all the predictors for hierarchical model with pClass and isMale.
* **Model 14:** Double grouping for hierarchical model with only fareSqrt predictor and double-grouped by pClass and title.
* **Model 15:** Double grouping all the predictors for hierarchical model with pClass and Cabin.

```{r additional models, cache=TRUE, eval=TRUE, echo=FALSE, message=FALSE, warning = FALSE, error=FALSE, results="hide"}

# Interaction terms added to all predictors with population terms only model
formula_all_population_interactions <- as.formula(y.train ~ 1 + Pclass2 + Pclass3 + Age + Ticket5 + Ticket6 + TicketOther + CabinB + CabinC + CabinD + CabinE + CabinF + CabinG + CabinT + EmbarkedQ + EmbarkedS + titleMr + titleMrs + titleOther + fSizesingleton + fSizesmall + isMale1 + FareSqrt + fSizesingleton:FareSqrt + Pclass3:Age + FareSqrt:Age + I(Age^2) + CabinT:EmbarkedQ + TicketOther:isMale1)
...
formula_multi_hier1_title_interactions <- as.formula(y.train ~ 1 + Pclass2 + Pclass3 + Age + Ticket5 + Ticket6 + TicketOther + CabinB + CabinC + CabinD + CabinE + CabinF + CabinG + CabinT + EmbarkedQ + EmbarkedS + fSizesingleton + fSizesmall + isMale1 + FareSqrt + fSizesingleton:FareSqrt + Pclass3:Age + FareSqrt:Age + I(Age^2) + CabinT:EmbarkedQ + TicketOther:isMale1 + (1 + Pclass2 + Pclass3 + Age + Ticket5 + Ticket6 + TicketOther + CabinB + CabinC + CabinD + CabinE + CabinF + CabinG + CabinT + EmbarkedQ + EmbarkedS + fSizesingleton + fSizesmall + isMale1 + FareSqrt | title))
...
formula_multi_hier2_pClass_interactions <- as.formula(y.train ~ 1 + Age + Ticket5 + Ticket6 + TicketOther + CabinB + CabinC + CabinD + CabinE + CabinF + CabinG + CabinT + EmbarkedQ + EmbarkedS + titleMr + titleMrs + titleOther + fSizesingleton + fSizesmall + isMale1 + FareSqrt + fSizesingleton:FareSqrt + titleOther:Age + FareSqrt:Age + I(Age^2) + CabinT:EmbarkedQ + TicketOther:isMale1 + (1 + Age + Ticket5 + Ticket6 + TicketOther + CabinB + CabinC + CabinD + CabinE + CabinF + CabinG + CabinT + EmbarkedQ + EmbarkedS + titleMr + titleMrs + titleOther + fSizesingleton + fSizesmall + isMale1 + FareSqrt | pClass))
...
# Interaction terms added to all predictors with hierarchical model grouped by Cabin
formula_multi_hier3_Cabin_interactions <- as.formula(y.train ~ 1 + Pclass2 + Pclass3 + Age + Ticket5 + Ticket6 + TicketOther + EmbarkedQ + EmbarkedS + titleMr + titleMrs + titleOther + fSizesingleton + fSizesmall + isMale1 + FareSqrt + fSizesingleton:FareSqrt + titleOther:Age + FareSqrt:Age + I(Age^2) + TicketOther:isMale1 + (1 + Pclass2 + Pclass3 + Age + Ticket5 + Ticket6 + TicketOther + EmbarkedQ + EmbarkedS + titleMr + titleMrs + titleOther + fSizesingleton + fSizesmall + isMale1 + FareSqrt | Cabin))

formula_multi_hier3_Cabin_interactions <- bf(formula = formula_multi_hier3_Cabin_interactions, center = F)

multi_hier3_Cabin_post_interactions.fit <- brm(formula = formula_multi_hier3_Cabin_interactions,
                                       data = df3.train,
                                       prior = prior0,
                                       family = bernoulli(link="logit"),
                                       cores = parallel::detectCores(),
...
formula_hier4_doubleClass <- as.formula(paste("y.train ~", paste0("1+",paste(colnames(df4.train)[2:(length(colnames(df4.train))-2)], collapse = "+")),"+(1+",paste(colnames(df4.train)[2:(length(colnames(df4.train))-2)], collapse = "+"),"|(ticket:embarked))"))
...
formula_hier5_doubleClass <- as.formula(paste("y.train ~", paste0("1+",paste(colnames(df5.train)[2:(length(colnames(df5.train))-2)], collapse = "+")),"+(1+",paste(colnames(df5.train)[2:(length(colnames(df5.train))-2)], collapse = "+"),"|(pClass:isMale))"))
...
formula_hier6_doubleClass <- as.formula(y.train ~ 1 + FareSqrt + (1 + FareSqrt | pClass:title))
formula_hier6_doubleClass <- bf(formula = formula_hier6_doubleClass, center = F)

multi_hier6_doubleClass.fit <- brm(formula = formula_hier6_doubleClass,
                                      data = df6.train,
                                      prior = prior0,
                                      family = bernoulli(link="logit"),
                                      cores = parallel::detectCores(),
...
# Double grouping for hierarchical model with pClass and Cabin
formula_hier7_doubleClass <- as.formula(paste("y.train ~", paste0("1+",paste(colnames(df7.train)[2:(length(colnames(df7.train))-2)], collapse = "+")),"+(1+",paste(colnames(df7.train)[2:(length(colnames(df7.train))-2)], collapse = "+"),"|(pClass:Cabin))"))
formula_hier7_doubleClass <- bf(formula = formula_hier7_doubleClass, center = F)

multi_hier7_doubleClass.fit <- brm(formula = formula_hier7_doubleClass,
                                      data = df7.train,
                                      prior = prior0,
                                      family = bernoulli(link="logit"),
                                      cores = parallel::detectCores(),
...
We will not go into posterior predictive checks for each model but instead compare the predictive ability of all the models using leave-one-out cross validation.

## 7 - Model Comparison

In this section we compare the predictive performance of the models using leave-one-out cross validation method. The comparison can be seen below with the best model being on the top of the list:

```{r model comparison1, cache=TRUE, eval=TRUE, echo=FALSE, message=FALSE, warning = FALSE, error=FALSE, results="hide"}
loo1 <- brms::loo(only_population_intercept.fit, save_psis = TRUE, cores = 4, reloo = F)
loo2 <- brms::loo(population_all.fit, save_psis = TRUE, cores = 4, reloo = F)
loo3 <- brms::loo(population_all_hs.fit, save_psis = TRUE, cores = 4, reloo = F)
loo4 <- brms::loo(population_and_group_intercept.fit, save_psis = TRUE, cores = 4, reloo = F)
loo5 <- brms::loo(multi_hier1_title_post.fit, save_psis = TRUE, cores = 4, reloo = F)
loo6 <- brms::loo(multi_hier2_pClass_post.fit, save_psis = TRUE, cores = 4, reloo = F)
loo7 <- brms::loo(multi_hier3_Cabin_post.fit, save_psis = TRUE, cores = 4, reloo = F)
loo8 <- brms::loo(population_all_interactions.fit, save_psis = TRUE, cores = 4, reloo = F)
loo9 <- brms::loo(multi_hier1_title_post_interactions.fit, save_psis = TRUE, cores = 4, reloo = F)
loo10 <- brms::loo(multi_hier2_pClass_post_interactions.fit, save_psis = TRUE, cores = 4, reloo = F)
loo11 <- brms::loo(multi_hier3_Cabin_post_interactions.fit, save_psis = TRUE, cores = 4, reloo = F)
loo12 <- brms::loo(multi_hier4_doubleClass.fit, save_psis = TRUE, cores = 4, reloo = F)
loo13 <- brms::loo(multi_hier5_doubleClass.fit, save_psis = TRUE, cores = 4, reloo = F)
loo14 <- brms::loo(multi_hier6_doubleClass.fit, save_psis = TRUE, cores = 4, reloo = F)
loo15 <- brms::loo(multi_hier7_doubleClass.fit, save_psis = TRUE, cores = 4, reloo = F)
```

```{r model comparison2, cache=TRUE, eval=FALSE, echo=FALSE, message=FALSE, warning = FALSE, error=FALSE, results="hide"}
...
#plot(loo13, label_points = TRUE)
#plot(loo14, label_points = TRUE)
#plot(loo15, label_points = TRUE)
```

```{r model comparison3, cache=TRUE, eval=TRUE, echo=FALSE, message=FALSE, warning = FALSE, error=FALSE}
brms::loo_compare(loo1,loo2,loo3,loo4,loo5,loo6,loo7,loo8,loo9,loo10,loo11,loo12,loo13,loo14,loo15)
# using list of loo objects
#loo_list <- list(loo1,loo2,loo3,loo4,loo5,loo6,loo7,loo8,loo9,loo10, loo11,loo12)
#loo_model_weights(loo_list)
```

According to the comparison above according the loo log likelihood values, the double group model according to isMale and pClass predictors seem to be giving the best predictive performance whereas the only population level intercept model seems to be giving the worst performance. We can also make a comment that there is not a big difference between the predictive performance of the first ten models and the rankings can change if re-fitting is included in the loo analysis.

Next we will perform the prediction on the test set.

## 8 - Prediction Submission and Conlcusions

In this section, we will make predictions for the top 5 models according to loo analysis on the test set and record the actual accuracy. We will also make a prediction with an extra model that is the average of these 5 models using bayesian model averaging. We will inlcude the loo scores and the prediction results in the same table below: 

```{r predictions1, cache=TRUE, eval=TRUE, echo=FALSE, message=FALSE, warning = FALSE, error=FALSE}
title <- titleTest
pClass <- pClassTest
ticket <- ticketTest
...
df.preds <- cbind(df0.test,title,pClass,ticket,embarked,isMale,Cabin)

probs_all <- brms::pp_average(multi_hier4_doubleClass.fit,
                              multi_hier5_doubleClass.fit,
                              multi_hier6_doubleClass.fit,
                              multi_hier1_title_post.fit,
                              population_all_hs.fit,
...
```{r predictions2, cache=TRUE, eval=TRUE, echo=FALSE, message=FALSE, warning = FALSE, error=FALSE}
# prediction with model13
y_pred13<- posterior_predict(multi_hier5_doubleClass.fit, df.preds, allow_new_levels = T)
prob13 <- colMeans(y_pred13)
pred13 <- as.integer(prob13 >= 0.5)

model13.submission <- data.frame("PassengerId" = pId.test, "Survived" = pred13)
write.csv(x = model13.submission, file = file.path("C:","Users","alpgu","Dropbox","Data Science",                                             "Bayesian Data Analysis","BDA3 (Gelman)","titanic","submissions","model13.csv"),
          row.names = FALSE)

# prediction with model3
y_pred3<- posterior_predict(population_all_hs.fit, df.preds, allow_new_levels = T)
prob3 <- colMeans(y_pred3)
pred3 <- as.integer(prob3 >= 0.5)

model3.submission <- data.frame("PassengerId" = pId.test, "Survived" = pred3)
write.csv(x = model3.submission, file = file.path("C:","Users","alpgu","Dropbox","Data Science",                                             "Bayesian Data Analysis","BDA3 (Gelman)","titanic","submissions","model3.csv"),
          row.names = FALSE)

# prediction with model14
y_pred14<- posterior_predict(multi_hier6_doubleClass.fit, df.preds, allow_new_levels = T)
prob14 <- colMeans(y_pred14)
pred14 <- as.integer(prob14 >= 0.5)

model14.submission <- data.frame("PassengerId" = pId.test, "Survived" = pred14)
write.csv(x = model14.submission, file = file.path("C:","Users","alpgu","Dropbox","Data Science",                                             "Bayesian Data Analysis","BDA3 (Gelman)","titanic","submissions","model14.csv"),
          row.names = FALSE)

# prediction with model12
y_pred12<- posterior_predict(multi_hier4_doubleClass.fit, df.preds, allow_new_levels = T)
prob12 <- colMeans(y_pred12)
pred12 <- as.integer(prob12 >= 0.5)

model12.submission <- data.frame("PassengerId" = pId.test, "Survived" = pred12)
write.csv(x = model12.submission, file = file.path("C:","Users","alpgu","Dropbox","Data Science",                                             "Bayesian Data Analysis","BDA3 (Gelman)","titanic","submissions","model12.csv"),
          row.names = FALSE)

# prediction with model5
y_pred5<- posterior_predict(multi_hier1_title_post.fit, df.preds, allow_new_levels = T)
prob5 <- colMeans(y_pred5)
pred5 <- as.integer(prob5 >= 0.5)

model5.submission <- data.frame("PassengerId" = pId.test, "Survived" = pred5)
write.csv(x = model5.submission, file = file.path("C:","Users","alpgu","Dropbox","Data Science",                                             "Bayesian Data Analysis","BDA3 (Gelman)","titanic","submissions","model5.csv"),
          row.names = FALSE)
```

```{r predictions3, cache=TRUE, eval=TRUE, echo=FALSE, message=FALSE, warning = FALSE, error=FALSE}
model_acc <- data.frame(ModelName = as.character(), Loo = as.numeric(), ActualAcc = as.numeric())
model_acc[1,1] <- "Model BMA"
model_acc[1,2] <- NaN
model_acc[1,3] <- 0.77272
model_acc[2,1] <- "Model13"
model_acc[2,2] <- 0
model_acc[2,3] <- 0.77511
model_acc[3,1] <- "Model3"
model_acc[3,2] <- -3.2
model_acc[3,3] <- 0.77990
model_acc[4,1] <- "Model14"
model_acc[4,2] <- -3.3
model_acc[4,3] <- 0.76794
model_acc[5,1] <- "Model12"
model_acc[5,2] <- -3.4
model_acc[5,3] <- 0.78229
model_acc[6,1] <- "Model5"
model_acc[6,2] <- -3.6
model_acc[6,3] <- 0.77272

model_acc %>% kbl() %>% kable_styling(latex_options = "hold_position")
```

As can be seen from the results, the predictive performance assessed with loo didn't perfectly reflect the actual predictive performance of the models. The reason could be that the log-likelihood of the models on the training set are very close to each other and that the remodelling of the levarage points with high pareto-k values have not been remodelled. It could have fine tuned the log-likelihood comparison and could have changed the predictive performance ranking.

From the current analysis, it can be seen that the hierarchical model grouped by title gave the best results. Nevertheless, even the best model was able to predict around 78% of the survived passengers. Further non-linear models can be developed to improve performance.

It should be noted that one of the best models is the non-hierarchical one with horseshoe prior. Considering the amount of computing power and time needed to perform the whole analysis, the horseshoe prior provides a very good alternative to the rest of the models.