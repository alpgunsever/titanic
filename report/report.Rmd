...
bayes_R2(only_population_intercept.fit)
bayes_R2(population_all.fit)
bayes_R2(population_all_hs.fit)
bayes_R2(population_and_group_intercept.fit)
bayes_R2(multi_hier1_title_post.fit)
bayes_R2(multi_hier2_pClass_post.fit)
bayes_R2(multi_hier3_Cabin_post.fit)
```

```{r diagnostics1, cache=TRUE, eval=FALSE, echo=FALSE}
mcmc_areas(population_all_hs.fit, pars = c("b_Intercept",paste0("b_",colnames(df0.train)[2:(length(colnames(df0.train))-1)])))
```
...
pp_check(y.train, yrep7.posterior[1:50,], ppc_dens_overlay)
