...
pp_check(y.train, yrep.prior1, ppc_dens_overlay, nsamples = 100)
...
y_pred2 <- posterior_predict(population_all.fit, df1.test)
y_pred3 <- posterior_predict(population_all_hs.fit, df1.test)
...
pp_check(y.train, yrep1.posterior, ppc_dens_overlay, nsamples = 50)
pp_check(y.train, yrep2.posterior, ppc_dens_overlay, nsamples = 50)
pp_check(y.train, yrep3.posterior, ppc_dens_overlay, nsamples = 50)
pp_check(y.train, yrep4.posterior, ppc_dens_overlay, nsamples = 50)
pp_check(y.train, yrep5.posterior, ppc_dens_overlay, nsamples = 50)
pp_check(y.train, yrep6.posterior, ppc_dens_overlay, nsamples = 50)

pp_check(y.train, yrep5.posterior, fun = "stat_grouped", group = train_imp$title, stat = "mean")

prop_zero <- function(x) mean(x == 1)
prop_zero(y.train) # check proportion of zeros in y
pp_check(y.train, yrep5.posterior, fun = "stat_grouped", stat = "prop_zero", group = train_imp$title)
...
# predictor selection for horseshoe prior model
refmodel <- get_refmodel(population_all_hs.fit)
...
brms::loo_compare(loo1,loo2,loo3,loo4,loo5,loo6,loo7)
```
