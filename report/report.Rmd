...
We will start imputing the missing observations in the Age predictor. We wil replace the missing observations with the median of each title group that the missing observation belongs to. It will add bias to the predictor and will make it approach the median value but having the median per group will also add group level variance to the predictor.

```{r imputation 2, cache=TRUE, eval=TRUE, echo=FALSE}
...
plot(imp_Age)
dat.full$Age <- complete(imp_Age)

#Imputing the age variable
#title_medAge <- dat.full.train %>% group_by(title) %>% summarise(n=n(),avFare = median(Age, na.rm = T))
#for (i in 1:length(dat.full$Age)){
#  if(is.na(dat.full$Age[i])==T){
#    if(dat.full$title[i] == "Master") dat.full$Age[i] = title_medAge[1,3]
#    if(dat.full$title[i] == "Miss") dat.full$Age[i] = title_medAge[2,3]
#    if(dat.full$title[i] == "Mr") dat.full$Age[i] = title_medAge[3,3]
#    if(dat.full$title[i] == "Mrs") dat.full$Age[i] = title_medAge[4,3]
#    if(dat.full$title[i] == "Noble") dat.full$Age[i] = title_medAge[5,3]
#    if(dat.full$title[i] == "Soldier") dat.full$Age[i] = title_medAge[6,3]
#  }
#}
dat.full$Age <- as.numeric(dat.full$Age)
```

Now we can have a look at the exploratory graphs for this predictor after imputation for all the missing observations is completed. The visualization will represent only the training set values as done before:
